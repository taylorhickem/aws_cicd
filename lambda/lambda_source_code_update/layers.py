...
class LayerManager(object):
    ...
    def lambda_layers_update(self, function_name):
        arns = self.layer_arns()
        return lambda_layers_update(function_name, arns)


def lambda_layers_update(function_name, layer_arns):
    args = {
    ...
    try:
        ...
        message = f'lambda {function_name} layers updated {layer_arns}'
        status_type = 1
    except Exception as e:
        message = f'ERROR. failed to update lambda {function_name} with layers {layer_arns}. {str(e)}'
        status_type = 0
